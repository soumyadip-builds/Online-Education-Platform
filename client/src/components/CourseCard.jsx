...
import { isAuthed } from '../lib/authLocal';
...
export default function CourseCard({ course }) {
	...
	const { id, title, author, isBestseller, rating, learners, thumbnail } = course;

	const handleClick = () => {
		// Auth check via authLocal (JWT + user)
			...
			<div className="card-body d-flex flex-column">
				<div className="d-flex justify-content-between align-items-start mb-2">
					<span className="badge bg-primary-subtle text-primary">
						{isBestseller ? (
							<span>
								<i className="bi bi-bookmark-star-fill"></i> Bestseller
							</span>
						) : (
							<span>
								<i className="bi bi-shield-fill-check"></i> Premium
							</span>
						)}
					</span>

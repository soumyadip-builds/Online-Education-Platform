...
export default function CourseDetails() {
        ...
        const normalizedCourse = {
          ...
          includes: {
            hoursOnDemandVideo: foundCourse?.totalEstimatedMinutes
              ? Math.round((foundCourse.totalEstimatedMinutes / 60) * 10) / 10
              : (foundCourse?.counts?.videos ?? 0),
            articles: foundCourse?.counts?.documentation ?? 0,
            downloadableResources: foundCourse?.counts?.assignments ?? 0,
            accessOnMobile: true,
            certificate: true,
          },
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      // Notification is now handled server-side in enrollCourse controller
      // No need to call notifyCourseEnrollment here
    } catch (e) {
      ...
      fetch(`${API_BASE}/courses/${id}`, {
        ...
        .then((payload) => {
          const isWrapped =
            ...
            typeof payload === "object" &&
            Object.prototype.hasOwnProperty.call(payload, "ok");
          const rawCourse = isWrapped && payload.ok ? payload.data : payload;
          ...
          <div className="resources">
              ...
              <section className="course-details__section">
                <div className="video-grid">
                  {videos.slice(0, 1).map((v) => (
                    <article className="video-card" key={v.id || v.title}>
            ...
            <section className="course-details__section">
              ...
              <ul className="includes-list list-unstyled">
                ...
                {includes.articles > 0 && (
                  <li>
                    <span className="icon">📰</span> {includes.articles}{" "}
                    articles
                  </li>
                )}

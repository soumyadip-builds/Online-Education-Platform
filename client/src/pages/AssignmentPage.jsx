...
function lsAssignmentsLoad() {
	try {
		return JSON.parse(localStorage.getItem(LS_ASSIGNMENTS_KEY) ?? '[]');
	} catch {
		return [];
	}
}
...
export default function AssignmentPage() {
	...
	useEffect(() => {
		...
		const prev =
			safeJSONParse(
				localStorage.getItem(`assignmentAttempt:${who}:${assignmentId}`),
			) ||
			safeJSONParse(
				localStorage.getItem(`assignmentResult:${who}:${assignmentId}`),
			) ||
			null;
		if (prev?.submittedAt) {
	...
	function persistAssignmentAttempt(payload) {
		try {
			...
			localStorage.setItem(
				`assignmentAttempt:${who}:${assignmentId}`,
			...
			localStorage.setItem(
				`assignmentAttemptFile:${who}:${assignmentId}`,
				JSON.stringify(payload),
			);

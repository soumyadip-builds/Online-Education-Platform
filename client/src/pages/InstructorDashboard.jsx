...
export default function InstructorDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/instructorDashboard/${instructorId}/dashboard`, {
          method: "GET",
          headers: {
        ...
        if (!res.ok) {
          ...
          throw new Error(`Failed to load dashboard (${res.status}): ${text || res.statusText}`);
        }
  ...
  const studentRows = useMemo(() => {
    ...
    return enrolledStudents.map((s) => {
      ...
      const completedTrackable = (m.assignmentsCompleted || 0) + (m.quizzesCompleted || 0);
      const pct =
        totalTrackable === 0
          ? 0
        ...
        <div style={{ marginTop: 16 }}>
          ...
          {courses.length === 0 ? (
            <div className="im-empty">
              <p className="im-muted">No courses found for you.</p>
              <p className="im-muted">Create a course to see learner progress here.</p>
            </div>
          ) : (
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <div className="im-muted">
                Trackables: <b>{totals.assignments}</b> assignments +{" "}
                <b>{totals.quizzes}</b> quizzes
              </div>
            </div>
          )}
        </div>

...
import { getCurrentUser } from '../utils/session'; // ← NEW: detect role
...
export default function CourseDetails() {
  ...
  // 🔐 Session user + role
  const sessionUser = getCurrentUser(); // { role, name, email, ... } or null
  const role = sessionUser?.role || 'guest'; // 'learner' | 'instructor' | 'guest'
  ...
  // Enrollment (original behavior kept; only visibility changes by role)
  ...
  if (loadErr || !course) {
    return (
      <div className="course-details">{loadErr || 'Course not found.'}</div>
    );
  }
        ...
        <section className="course-details__hero">
          <div>
               ...
               ========================= */}
            {
              ...
              ) : null /* Instructors (and others) see no Enroll UI */
            }
          </div>
                    ...
                    i === 0 ? (
                      <article className="video-card" key={i}>
                        <div className="video-thumb">
                          ...
                          {/* Lock overlay only makes sense for learners */}
                          {isLearner && !enrolled && (
                        ...
                        <div className="video-info">
                          ...
                          <a
                            ...
                            onClick={(e) => {
                              if (isLearner && !enrolled) e.preventDefault();
                            }}
                          >
                            ▶ Watch
                          </a>
              ...
              {sections.map((s, i) => (
                <li key={i}>
                  ...
                  {typeof s.lectures === 'number' && (
                    <span className="muted"> · {s.lectures} lectures</span>
                  )}
                  ...
                  <article className="video-card" key={i}>
                    <div className="video-info">
                      ...
                      <a
                        href={v.url}
                        className={`video-link ${
                          isLearner && !enrolled ? 'video-link--disabled' : ''
                        }`}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={(e) => {
              ...
              <ul className="resource-list">
                {links.map((l, i) => (
                  <li key={i}>
                    <a
                      className={`resource-link ${
                        isLearner && !enrolled ? 'resource-link--disabled' : ''
                      }`}
                      href={l.url}
                      target="_blank"
                      rel="noopener noreferrer"
